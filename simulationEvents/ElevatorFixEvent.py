...
class ElevatorFixEvent(Event):
    ...
    def treatEvent(self):
        if self.entity.state == ElevatorState.BROKEN:
            ...
            print(self.entity.name, "fixed at ", self.time)
            if self.simulatorManager.elevators[0].state != ElevatorState.BROKEN and self.simulatorManager.elevators[1].state != ElevatorState.BROKEN:
                newEvent = ElevatorOutOfServiceEvent(self.simulatorManager, self.simulatorManager.elevators[2], self.time)
